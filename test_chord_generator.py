...
def test_order_does_not_matter():
    assert (
            len(set(chord for chord, _ in chords)) ==
            len(set(tuple(sorted(set(chord))) for chord, _ in chords))
            )
...
def test_center_columns_are_the_only_key_pressed():
    for chord, _ in chords:
        for center_column, other_keys_on_hand in center_columns_and_corresponding_hands:
            ...
            assert pressed_center_column_keys == set() or other_pressed_keys_on_hand == set()
